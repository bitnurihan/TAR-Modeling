...
def non_linear_regression():
    ...
    # cost = np.array(df_2s_ad_data_final['Cost'])
    cost = np.linspace(0.1, max(df_2s_ad_data_final['Cost']), len(df_2s_ad_data_final['Cost']))
    reach = np.array(df_2s_ad_data_final['r1']) + 0.9*np.random.normal(size=len(cost))
    find_dgt = df_2s_ad_data_final[df_2s_ad_data_final['screen'] == 'DGT']
    if find_dgt is True:
        GRP = np.array(df_2s_ad_data_final['impression'])
    else:
        GRP = np.array(df_2s_ad_data_final['GRP'])
    n = len(df_2s_ad_data_final['Cost'])
    ...
    for i in range(n):
        cost_grp[i] = GRPs(cost[i], R_guess_grp[0], R_guess_grp[1])
        grp_r1[i] = reach1(cost_grp[i], R_guess_r1[0], R_guess_r1[1])

    # print('R^2 : ', r2_score(cost_grp, GRP))
    ...
    plt.plot(cost, grp_r1, 'r.', label='R_guess')  # R 계수에 대한 그래프
    plt.plot(cost, p_grp_r1, 'r.', color='b', label='Py_guess')
    plt.scatter(df_2s_ad_data_final['Cost'], df_2s_ad_data_final['r1'], color='y', label='Real data')
    plt.xlabel('Cost')
    plt.ylabel('reach1+')
...
screen = ['DGT']
with PdfPages('python_R_modeling.pdf') as pdf_pages:
    for age in ages:
        ...
        for i in screen:
            ...
            # print(age, i, len(df_2s_ad_data_final))
            # non_linear_regression()
        # corr = df_2s_ad_data_final.corr(method='pearson')
        # print(age, corr)
